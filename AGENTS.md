...
docs/        desktop architecture, build/release, quality, and screenshots (see docs/AGENTS.md)
...
pnpm run doc-sync       # project documentation gates; leaf list in scripts/run-gates.ts
...
Run the checks relevant to the changed surface before pushes; report only commands run. After any branch synchronization, validate immediately; do not merge before checks pass.
...
- `test:coverage`, not `test`, is the CI coverage gate; the desktop release checks are summarized in [docs/quality.md](docs/quality.md).

## Secrets / .env

Real-API tests and demos read `DEEPSEEK_API_KEY`, optional `DEEPSEEK_BASE_URL`, and root `.env`. cordis.yml allows `!!js` (never `!js`) under plugin `config` and entry `disabled`; other metadata stays literal, so conditional composition also uses overlays. Never commit credentials. CI e2e skips without a key.

## Conventions

- Every npm package is `@deepseek-ai/dsh-<name>`; vendored packages are rescoped and `private: true`. `@deepseek-ai/cordis` is a peerDependency (+ dev) of every harness package.
...
- **Non-trivial changes MUST include an Agent Note in the same PR;** only mechanical/local edits are exempt. Notes are kept concise and must describe the maintained decision.
- **Testing policy** — [docs/quality.md](docs/quality.md). Every non-trivial model- or product-user-visible behavior change adds or updates a keyless snapshot through a real runnable example in the same PR; package tests, e2e-only assertions, and mock-only fixtures do not substitute for the assembled application transcript. Fixtures must replay on macOS/Linux; fix fixtures, not normalizers.
...
- **Choose PR history deliberately.** Split independent changes; fix the introducing PR before propagation. Rewrites use `--force-with-lease`, abort on remote movement, never raw `--force`.
...
Read [docs/quality.md](docs/quality.md) before lifecycle, concurrency, subprocess, or teardown work.
...
Comments and docs state complete contracts and context, not reasoning transcripts. Use direct, concrete terms. Do not use metaphors. Before writing `contract`, `boundary`, or `shape`, ask whether a more exact term names the subject: write `response fields`, `JSON validation`, or `ESM exports` instead of `response shape`, `validation boundary`, or `module shape`. Keep `contract` for preconditions, postconditions, invariants, compatibility promises, and other obligations that callers, callees, implementers, providers, producers, or consumers rely on. Keep a literal process, wire, security, transaction, or lifecycle boundary. Do not narrate control flow or tests, preserve review history, or restate code. Keep behavior, failure, timing, ownership, and safe-use facts. Wire mechanically checkable invariants into an executed top-level gate and prove each changed acceptance path rejects an invalid case. Use narrow, justified exceptions instead of disabling a rule globally.
